...
class CoupledODEBlock(ODEblock):
  def __init__(self, odefunc, regularization_fns, opt, data, device, t=torch.tensor([0, 1])):
    super(CoupledODEBlock, self).__init__(odefunc, regularization_fns, opt, data, device, t)
  ...
  def forward(self, x):
    t = self.t.type_as(x)
    v = torch.ones(size=(x.size(0), x.size(1))).to(x)
    ...
    # added from constant block
    state = (state,) + reg_states if self.training and self.nreg > 0 else state
